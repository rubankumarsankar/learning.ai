...
export async function generateMetadata({ params }) {
  ...
  let desc = `Deep dive materials for ${slug}.`;
  try {
    ...
    if (resource.content) { desc = resource.content.replace(/[#*`\n\\]/g, ' ').substring(0, 150).trim() + '...'; }
  } catch (e) {}
  ...
  try {
    ...
    return (
      <div className="min-h-screen bg-black text-white p-10 font-mono flex flex-col items-center justify-center">
        ...
        <p className="text-gray-400 mb-8">The requested module &quot;{slug}&quot; does not exist.</p>
        <Link href="/learn/resources" className="bg-gray-800 border border-gray-600 px-6 py-2 rounded font-bold hover:bg-gray-700">← BACK TO LIBRARY</Link>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-10 font-mono">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 md:mb-8 gap-4">
        <Link href="/learn/resources" className="text-gray-400 hover:text-gray-200 font-bold">← BACK TO LIBRARY</Link>
        <div className="flex gap-3">
          <Link href="/learn/ask" className="text-green-400 hover:text-green-200 font-bold text-sm bg-gray-900 border border-green-800 px-3 py-1 rounded transition-colors">ASK AI 🧠 ↗</Link>
          {resource.url && <a href={resource.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-400 font-bold text-sm bg-gray-900 border border-blue-900 px-3 py-1 rounded transition-colors">OFFICIAL DOCS ↗</a>}
        </div>
      </div>
          ...
          <section className="bg-gray-800 p-5 md:p-8 rounded-xl border border-blue-600 shadow-lg shadow-blue-900/20">
            ...
            <ul className="space-y-3">
              {resource.keyTakeaways.map((pt, i) => (
                <li key={i} className="flex items-start gap-3 text-gray-200 leading-relaxed text-base md:text-lg"><span className="text-blue-500 mt-1">✓</span><span>{pt}</span></li>
              ))}
            </ul>
          </section>
        )}
        <section className="bg-gray-900 p-5 md:p-8 rounded-xl border border-gray-800">
          <div className="text-gray-300 leading-relaxed text-base md:text-lg overflow-x-auto">
            <ReactMarkdown components={{
              ...
              code: ({node, inline, ...props}) => inline ? <code className="bg-black text-pink-400 px-1 py-0.5 rounded text-sm" {...props} /> : <code className="block bg-black p-4 rounded text-sm my-4 overflow-x-auto text-blue-300" {...props} />
          ...
          <section className="bg-gray-900 p-5 md:p-8 rounded-xl border border-gray-800">
            <div className="bg-black p-4 md:p-6 rounded-lg overflow-x-auto border border-gray-700">
              <ReactMarkdown components={{
                ...
                code: ({node, ...props}) => <code className="text-blue-300 font-mono text-xs md:text-sm leading-loose" {...props} />
              }}>{resource.codeExample}</ReactMarkdown>
            </div>
          </section>
        )}
